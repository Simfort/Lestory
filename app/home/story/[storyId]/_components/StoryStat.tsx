...
export default function StoryStat({ story }: { story: TStory }) {
  ...
  const [likes, setLike] = useState(story.likes.length);
  const handleLike = async () => {
    try {
      const res = await fetch(`/api/story/like?storyId=${story.id}`, {
        method: "POST",
      });
      const data: { like: boolean } = await res.json();
      if (data.like) {
        setLike((prev) => prev + 1);
      }
    } catch (error) {
  ...
  return (
    <div className="p-5 flex bg-gray-50 shadow rounded-xl flex-col items-center gap-2">
      ...
      <div className="flex gap-5">
        ...
        <button
          onClick={handleLike}
          className="flex gap-2 hover:opacity-50 active:opacity-40 transition-opacity text-red-500 font-bold items-center">
          {likes}
          <Heart />
        </button>
      </div>
